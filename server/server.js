...
io.on('connection', (socket) => {
    ...
    socket.on('host-join', (data) => {
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            dbo.collection('buzzworld').find(query).toArray(function(err, result) {
                if (err) throw err;
    ...
    socket.on('host-join-game', (data) => {
        ...
        if (game) {
            ...
            MongoClient.connect(url, function(err, db) {
                ...
                dbo.collection("buzzworld").find(query).toArray(function(err, res) {
                    if (err) throw err;
                    ...
                    socket.emit('gameQuestions', {
                        ...
                        totalquesnum: res[0].questions.length,
            ...
            MongoClient.connect(url, function(err, db) {
                ...
                dbo.collection("buzzworld").find(query).toArray(function(err, res) {
                    ...
                    if (game.gameData.playersAnswered == playerNum.length) {
                        game.gameData.questionLive = false; //Question has been ended bcz all players answered under time
                        var playerData = players.getPlayers(game.hostId);
    ...
    socket.on('requestDbNames', function() {
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            var dbo = db.db('buzzworldDB');
            dbo.collection("buzzworld").find().toArray(function(err, res) {
                if (err) throw err;
    ...
    socket.on('newQuiz', function(data) {
        MongoClient.connect(url, function(err, db) {
            if (err) throw err;
            var dbo = db.db('buzzworldDB');
            dbo.collection('buzzworld').find({}).toArray(function(err, result) {
                if (err) throw err;
                ...
                var game = data;
                dbo.collection("buzzworld").insertOne(game, function(err, res) {
                    if (err) throw err;
                    db.close();
                });
                db.close();
